...
# ========================== mean_data_stack ==========================
#
def test_mean_data_stack():

    # Do a spectral stack

    ss = np.array([[1,2,3,7],[0,5,8,2],[2,9,4,6]])
    mean, unc, goodbad = mean_data_stack(ss,stderr=False)

    np.testing.assert_allclose(mean,[1,5.33333,5,5],0.001)
    np.testing.assert_allclose(unc,[1,3.51188,2.64575,2.64575],0.001)
    np.testing.assert_array_equal(goodbad,np.ones((3,4),dtype=int))


    # Do a spectral stack

    ss = np.array([[1,2,3,7],[0,5,8,2],[2,9,4,6]])
    mean, unc, goodbad = mean_data_stack(ss,stderr=True)

    np.testing.assert_allclose(mean,[1,5.33333,5,5],0.001)
    np.testing.assert_allclose(unc,[1,3.51188,2.64575,2.64575]/np.sqrt(3),0.001)
    np.testing.assert_array_equal(goodbad,np.ones((3,4),dtype=int))
    ...
    ss = np.array([[1,2,3,7],[0,5,8,2],[2,9,4,6]])
    var = np.array([[0.1,0.1,0.1,0.2],[0.1,0.2,0.2,0.1],[0.1,0.2,0.1,0.2]])
    mean, unc, goodbad = mean_data_stack(ss,weights=1/var)

    np.testing.assert_array_equal(mean,[1,4.5,4.4,4.25],0.001)
    np.testing.assert_allclose(unc,[0.182574,0.22360,0.2,0.2236068],0.001)
    np.testing.assert_array_equal(goodbad,np.ones((3,4),dtype=int))
