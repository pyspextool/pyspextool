...
def read_line_list(file, delta_to_microns=False):
    ...
    check_parameter("read_line_list", "delta_to_microns", delta_to_microns, "bool")
    ...
    lineinfo = {
        ...
        "wavelength": np.array(swave),
        "id": np.array(lineid),
        "delta_wavelength_left": np.array(lwin),
        "delta_wavelength_right": np.array(rwin),
        "fit_type": np.array(fittype),
        "num_parms": np.array(nterms),
    }

    if delta_to_microns:
        lineinfo["delta_wavelength_left"] = lineinfo["delta_wavelength_left"] / 1e4
        lineinfo["delta_wavelength_right"] = lineinfo["delta_wavelength_right"] / 1e4
...
def read_wavecal_file(file):
    ...
    try:
        hdul = fits.open(file)
        logging.info(f"Opened the wavecal file: {file}")
    except OSError as e:
        msg = f"""
        Could not open the wavecal file: {file}.
...
def write_wavecal_1d(
    ...
    if xdinfo is None:
        ...
        for i in range(norders):
            ...
            wavecal[z] = (
                poly_2d(
                    wavecal[z], omask[z], dispersion_degree, xdinfo["orderdeg"], coeffs
                )
                * scale
            )
    ...
    for i in range(ncoeffs):
        for j in range(ncoeffs):
            ...
            comment = str(i) + "," + str(j) + " (col,row) element of the covariance"
