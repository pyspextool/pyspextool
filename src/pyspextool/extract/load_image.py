...
def load_image(
    ...
    *output_files,
    ...
    if isinstance(files, list):
        ...
        check_parameter("load_image", "files[1]", files[1], ["str", "list", "int"])

    check_parameter("load_image", "flat_name", flat_name, "str")

    check_parameter("load_image", "wavecal_name", wavecal_name, ["NoneType", "str"])
    ...
    check_parameter("load_image", "linearity_correction", linearity_correction, "bool")
    ...
    check_parameter("load_image", "qa_showsize", qa_showsize, ["NoneType", "tuple"])
    ...
    if isinstance(files, str):
        ...
        output_files = make_full_path(setup.state["proc_path"], [output_files[0]])
        ...
        input_files = make_full_path(path, nums, indexinfo=indexinfo, exist=True)
        ...
        output_files = make_full_path(
            setup.state["proc_path"], nums, indexinfo=indexinfo
        )
    ...
    if do_all_steps is False:
        ...
        if extract.state["modename"] != flatinfo["mode"]:
            ...
            extract.state["psdoorders"] = np.ones(flatinfo["norders"], dtype=int)
            extract.state["xsdoorders"] = np.ones(flatinfo["norders"], dtype=int)
        ...
        if dowavecal is True:
            ...
            fullpath = glob.glob(
                os.path.join(setup.state["package_path"], "data", "atran*.fits")
            )
            ...
            try:
                ...
                logging.info(f"Loaded the atmospheric transmission file: {fullpath}")
            except OSError as e:
                msg = f"""
                Could not open the atran file: {fullpath}.
            ...
            wavecal, spatcal = simulate_wavecal_1dxd(
                flatinfo["ncols"],
                flatinfo["nrows"],
                flatinfo["edgecoeffs"],
                flatinfo["xranges"],
    ...
    if verbose is True:
        ...
        if linearity_correction is True:
            message = "Loading " + file_names + " and correcting for non-linearity..."
            print(message)

        else:
            message = (
                "Loading " + file_names + " and not correcting for non-linearity..."
            )
            print(message)

    if reduction_mode == "A":
        if directory == "raw":
            img, var, hdrinfo, mask = instr.read_fits(
                input_files,
    ...
    extract.state["bad_pixel_mask"] = idl_rotate(
        setup.state["raw_bad_pixel_mask"], extract.state["rotation"]
    )
    ...
    for i in range(extract.state["norders"]):
        ...
        bot = np.ceil(
            poly_1d(indices[i]["x"], extract.state["edgecoeffs"][i, 0, :])
        ).astype(int)
