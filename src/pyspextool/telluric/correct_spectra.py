    ...
    for i in range(tc.state['object_norders']):
       ...
       # Now loop over the apertures
        ...
        for j in range(tc.state['object_napertures']):
            ...
            tc_w= np.squeeze(tc.state['shiftedtc_spectra'][k,0,:])
            tc_f = np.squeeze(tc.state['shiftedtc_spectra'][k,1,:])
            tc_u = np.squeeze(tc.state['shiftedtc_spectra'][k,2,:]) 
            tc_m = np.squeeze(tc.state['shiftedtc_spectra'][k,3,:])
