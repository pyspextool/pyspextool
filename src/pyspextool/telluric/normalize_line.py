...
from pyspextool.telluric.qaplots import plot_normalize_line
...
def normalize_line(
    ...
    check_parameter('prepare_line', 'order', 
                    order, 'int')

    check_parameter('prepare_line', 'line_wavelength', 
                    line_wavelength, 'float')

    check_parameter('prepare_line', 'resolving_power', 
                    resolving_power, ['int', 'float'])
    
    check_parameter('prepare_line', 'wavelength_range', 
                    wavelength_range, ['ndarray','list','tuple'])
    
    check_parameter('prepare_line', 'fit_type', 
                    fit_type, 'str',
                    possible_values=['gaussian','lorentzian'])    
    
    check_parameter('prepare_line', 'poly_degree', 
                    poly_degree, 'int')

    check_parameter('prepare_line', 'verbose', 
                    verbose, ['NoneType','bool'])

    check_parameter('prepare_line', 'qa_show', 
                    qa_show, ['NoneType','bool'])

    check_parameter('prepare_line', 'qa_showscale', 
                    qa_showscale, ['NoneType','float','int'])

    check_parameter('prepare_line', 'qa_showblock', 
                    qa_showblock, ['NoneType','bool'])
    
    check_parameter('prepare_line', 'qa_write', 
    ...
    config.state['normalizedline_wavelengths'] = wavelength
    config.state['normalizedline_fluxes'] = normalized_flux
    config.state['normalizedline_linecenter'] = line_center

    if fit_type == 'gaussian':

        config.state['normalizedline_fwhm'] = 2.354*line_halfwidth

    else:

        config.state['normalizedline_fwhm'] = 2*line_halfwidth        
    ...
    config.state['normalize_done'] = True
