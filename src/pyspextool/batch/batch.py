...
def processFolder(folder, verbose=False):
    ...
    for i, f in enumerate(files):
        ...
        for k in list(HEADER_DATA.keys()):
            ...
            for ii in HEADER_DATA[k]:
                if ii in list(header.keys()) and ref == "":
                    ref = ii
            if ref != "":
                dp[k].iloc[i] = header[ref]
            if ref == "" and verbose == True and i == 0:
                print("Could not find keywords {} in file {}".format(HEADER_DATA[k], f))
        # update some of the mode names
        if "SHORTXD" in dp["MODE"].iloc[i].upper():
            dp["MODE"].iloc[i] = "SXD"
        if "lowres" in dp["MODE"].iloc[i].lower():
            dp["MODE"].iloc[i] = "Prism"
        ...
        if dp["TARGET_TYPE"].iloc[i] == "":
            ...
            if "SXD" in dp["MODE"].iloc[i] and float(dp["INTEGRATION"].iloc[i]) <= 30.0:
                dp["TARGET_TYPE"].iloc[i] = "standard"
            if (
                "Prism" in dp["MODE"].iloc[i]
                and float(dp["INTEGRATION"].iloc[i]) <= 10.0
            ):
                dp["TARGET_TYPE"].iloc[i] = "standard"
    ...
    if len(dpt) == 0:
        if verbose == True:
            print(
                "Warning: no standards present in list; be sure to check driver file carefully"
            )
    ...
    dp["DATETIME"] = [
        dp["UT_DATE"].iloc[i] + "T" + dp["UT_TIME"].iloc[i] for i in range(len(dp))
    ]
    dp["MJD"] = [Time(d, format="isot", scale="utc").mjd for d in dp["DATETIME"]]
    dp["INSTRUMENT"] = ["spex"] * len(dp)
    if dp["MJD"].iloc[0] > INSTRUMENT_DATE_SHIFT:
        dp["INSTRUMENT"] = ["uspex"] * len(dp)
    ...
    if len(names) == 0:
        ...
        for n in names:
            ...
            if len(dpsa) > 1:
                pos1 = SkyCoord(
                    dpsa["RA"].iloc[0] + " " + dpsa["DEC"].iloc[0],
                    unit=(u.hourangle, u.deg),
                )
                pos2 = SkyCoord(
                    dpsa["RA"].iloc[-1] + " " + dpsa["DEC"].iloc[-1],
                    unit=(u.hourangle, u.deg),
                )
                ...
                if dx.value > MOVING_MAXSEP or (
                    (dx / dt).value > MOVING_MAXRATE and dt.value > 0.1
                ):
                    dp.loc[dp["TARGET_NAME"] == n, "FIXED-MOVING"] = "moving"
                if verbose == True:
                    print(
                        "{}: dx={:.1f} arc, dt={:.1f} hr, pm={:.2f} arc/hr = {}".format(
                            n,
                            dx,
                            dt,
                            dx / dt,
                            dp.loc[dp["TARGET_NAME"] == n, "FIXED-MOVING"],
                        )
                    )
...
def organizeLegacy(
    folder, outfolder="", verbose=ERROR_CHECKING, makecopy=False, overwrite=False
):
    """
    Purpose
    -------
    ...
    if not os.path.exists(base_folder):
        print(
            "Error: {} folder not found in the current directory, no action".format(
                LEGACY_FOLDER
            )
        )
        return
    ...
    for i, f in enumerate(fits_folders):
        ...
        if outfolder == "":
            ...
            for k in HEADER_DATA["UT_DATE"]:
                if k in list(hd.keys()):
                    prefix = hd[k].replace("/", "").replace("-", "")
        ...
        if os.path.exists(data_folder) == False:
            if makecopy == False:
                ...
                shutil.copytree(f, data_folder)
            if verbose == True:
                print("Moved legacy data folder {} to {}".format(f, data_folder))
        else:
            if len(glob.glob(os.path.join(f, "*.fits"))) == 0 or overwrite == True:
                for df in glob.glob(os.path.join(f, "*.fits")):
                    if makecopy == False:
                        shutil.move(df, data_folder)
                    else:
                        shutil.copy2(df, data_folder)
                if verbose == True:
                    print(
                        "Moved fits files in legacy data folder {} to {}".format(
                            f, data_folder
                        )
                    )
            else:
                print(
                    "WARNING: fits files already exist in {}; move these files or set overwrite to True".format(
                        data_folder
                    )
                )
        ...
        for rf in REDUCTION_FOLDERS[1:]:
            ...
            if os.path.exists(rfolder) == False:
                os.makedirs(rfolder)
                if verbose == True:
                    print("Creating folder {}".format(rfolder))
...
def writeLog(dp, log_file="", options={}, verbose=ERROR_CHECKING):
    ...
    if log_file != "":
        parameters["FILENAME"] = log_file

    # downselect columns to save out; by default
    if len(parameters["COLUMNS"]) > 0:
        try:
            dpout = dp[parameters["COLUMNS"]]
        except:
            dpout = copy.deepcopy(dp)
            if verbose == True:
                print(
                    "Warning: could not select subset of columns {} from data frame, saving all columns".format(
                        parameters["COLUMNS"]
                    )
                )
    else:
    ...
    if "TARGET_NAME" in list(dpout.columns) and "TARGET_TYPE" in list(dpout.columns):
        ...
        if len(tnames) == 1 and len(dpouts) > 50:
            if verbose == True:
                print(
                    "WARNING: only one target name {} suggesting an issue with labeling".format(
                        tnames[0]
                    )
                )
            if "RA" in list(dpout.columns) and "DEC" in list(dpout.columns):
                ...
                if verbose == True:
                    print("Replaced target names with coordinate short names")
    ...
    try:
        if "RA" in list(dpout.columns) and "DEC" in list(dpout.columns):
            ...
            for i, tnm in enumerate(tnames):
                ...
                if len(dpsel) > 0:
                    src_coord = SkyCoord(
                        dpsel["RA"].iloc[0] + " " + dpsel["DEC"].iloc[0],
                        unit=(u.hourangle, u.deg),
                    )
                    t = Table(
                        [[src_coord.ra.deg], [src_coord.dec.deg]], names=("ra", "dec")
                    )
                    t_match = XMatch.query(
                        t,
                        "simbad",
                        SIMBAD_RADIUS,
                        colRA1="ra",
                    ...
                    if len(t_match) > 0:
                        for x in list(SIMBAD_COLS.keys()):
                            dpout.loc[dpout["TARGET_NAME"] == tnm, x] = t_match[
                                SIMBAD_COLS[x]
                            ][0]
    except:
        print(
            "WARNING: Unable to match targets to SIMBAD via XMatch; check internet connection"
        )
    ...
    if ftype in ["xls", "xlsx"]:
        dpout.to_excel(parameters["FILENAME"], index=False)
    elif ftype in ["csv"]:
        dpout.to_csv(parameters["FILENAME"], sep=",", index=False)
    elif ftype in ["tsv", "txt", "tab"]:
        dpout.to_csv(parameters["FILENAME"], sep="\t", index=False)
    elif ftype in ["psv"]:
        dpout.to_csv(parameters["FILENAME"], sep="|", index=False)
    elif ftype in ["tex"]:
        dpout.to_latex(parameters["FILENAME"], longtable=True, index=False)
    elif ftype in ["json"]:
        dpout.to_json(parameters["FILENAME"])
    elif ftype in ["htm", "html"]:
        dpout.style.apply(logTablestyle)
        if parameters["HTML_TEMPLATE_FILE"] != "" and os.path.exists(
            parameters["HTML_TEMPLATE_FILE"]
        ):
            with open(parameters["HTML_TEMPLATE_FILE"], "r") as f:
                dphtml = f.read()
        else:
            dphtml = copy.deepcopy(parameters["BASE_HTML"])
        for x in ["INSTRUMENT", "UT_DATE"]:
            if x in list(dp.columns):
                dphtml = dphtml.replace("[{}]".format(x), dp[x].iloc[0])
        dphtml = dphtml.replace(
            "[TABLE]",
            dpout.to_html(
                classes="table table-striped text-center", index=False, bold_rows=True
            ),
        )
        ...
        raise ValueError(
            "Could not write out to {}; unknown file format".format(
                parameters["FILENAME"]
            )
        )

    if verbose == True:
        print("log written to {}".format(parameters["FILENAME"]))
    return
...
def readDriver(driver_file, options={}, verbose=ERROR_CHECKING):
    ...
    if os.path.exists(driver_file) == False:
        raise ValueError(
            "Cannot find batch driver file {}; check path".format(driver_file)
        )
    ...
    for i, sline in enumerate(slines):
        ...
        if len(rpars) < len(list(OBSERVATION_PARAMETERS_REQUIRED.keys())):
            if verbose == True:
                print(
                    "Warning: line\n{}\ncontains fewer than the required parameters\n{}; skipping".format(
                        sline, OBSERVATION_PARAMETERS_REQUIRED
                    )
                )
        else:
            for ii, k in enumerate(list(OBSERVATION_PARAMETERS_REQUIRED.keys())):
                spar[k] = rpars[ii + 1]
            # add optional parameters
            opars = list(filter(lambda x: ("=" in x), rpars))
            new_opar = dict([l.strip().split("=") for l in opars])
            for k in list(new_opar.keys()):
                new_opar[k.upper()] = new_opar.pop(k)
            ...
            if (
                parameters["INSTRUMENT"] == "spex"
                and "SXD" in spar["MODE"]
                and spar["ORDERS"][-1] == "9"
            ):
    ...
    for k in ["qa_write", "qa_show"]:
        if k in list(parameters.keys()):
            parameters[k] = bool(parameters[k])
...
def writeDriver(
    ...
    if data_folder != "":
        driver_param["DATA_FOLDER"] = data_folder

    # check folders
    if os.path.exists(driver_param["DATA_FOLDER"]) == False:
        raise ValueError(
            "Cannot find data folder {}: you must include this in batch driver file".format(
                driver_param["DATA_FOLDER"]
            )
        )
    for x in ["CALS_FOLDER", "PROC_FOLDER", "QA_FOLDER"]:
        ...
        if driver_param[x] == "":
            ...
            driver_param[x] = os.path.join(
                os.path.split(driver_param["DATA_FOLDER"])[0], (x.split("_")[0]).lower()
            )
        ...
        if os.path.exists(driver_param[x]) == False:
            if create_folders == True:
                ...
                if verbose == True:
                    print("Created {} folder {}".format(x, driver_param[x]))
            else:
                raise ValueError(
                    "Folder {} does not exist; either create or set keyword create_folders to True".format(
                        driver_param[x]
                    )
                )
    ...
    dpc["PREFIX"] = [
        x.replace(".a.fits", "").replace(".b.fits", "") for x in dpc["FILE"]
    ]
    n = 4
    if driver_param["INSTRUMENT"] == "uspex":
        n = 5
    ...
    try:
        ...
        raise ValueError(
            "Cannot write to {}, check file path and permissions".format(driver_file)
        )
    f.write(
        "# Batch reduction driver file for {} observations on {}\n".format(
            dpc["INSTRUMENT"].iloc[0], dpc["UT_DATE"].iloc[0]
        )
    )
    ...
    for x in [
        ...
        "CALIBRATED_FILE_PREFIX",
        "STITCHED_FILE_PREFIX",
    ]:
        f.write("{} = {}\n".format(x, str(driver_param[x])))
    ...
    for i in range(len(calf1)):
        cal_sets += "{:.0f}-{:.0f},".format(calf1[i], calf2[i])
    # 	f.write('CAL_SETS = {}\n'.format(cal_sets[:-1]))
    ...
    if len(tnames) == 0:
        if verbose == True:
            print(
                "Warning: no science files identified; you may need to update the observational logs"
            )
        f.close()
        return
    ...
    for n in tnames:
        ...
        src_coord = SkyCoord(
            dps["RA"].iloc[0] + " " + dps["DEC"].iloc[0], unit=(u.hourangle, u.deg)
        )

        # loop over modes, dropping ignored modes
        srcmodes = list(set(list(dps["MODE"])))
        for ignm in IGNORE_MODES:
            srcmodes = [i for i in srcmodes if i != ignm]
        srcmodes.sort()
        for m in srcmodes:
            ...
            line = "{}\t{}\t{}-ps\t{}\t{}".format(
                ...
                str(m),
                str(dpsrc["FIXED-MOVING"].iloc[0]),
                n,
                str(dpsrc["PREFIX"].iloc[0]),
            )
            ...
            if len(dpcals) == 0:
                if verbose == True:
                    print(
                        "WARNING: no calibration files associated with mode {} for source {}".format(
                            dps["MODE"].iloc[0], n
                        )
                    )
                cs = cal_sets.split(",")[0]
            ...
            ftxt = "\t{}\tUNKNOWN\t0-0\tflat{}.fits\twavecal{}.fits".format(
                str(driver_param["SCIENCE_FILE_PREFIX"]),
                cal_sets.split(",")[ical],
                cal_sets.split(",")[ical],
            )
            if len(dpflux) == 0:
                ...
                dpflux["DIFF1"] = [
                    np.abs(float(x) - np.nanmedian(dpsrc["AIRMASS"])) / 0.2
                    for x in dpflux["AIRMASS"]
                ]
                dpflux["DIFF2"] = [
                    np.abs(float(x) - np.nanmedian(dpsrc["MJD"])) * 12.0
                    for x in dpflux["MJD"]
                ]
                dpflux["DIFF3"] = [
                    src_coord.separation(
                        SkyCoord(
                            dpflux["RA"].iloc[i] + " " + dpflux["DEC"].iloc[i],
                            unit=(u.hourangle, u.deg),
                        )
                    )
                    .to(u.arcsecond)
                ...
                if len(fnum) < 2:
                    if verbose == True:
                        print(
                            "Fewer than 2 flux calibrator files for source {}".format(
                                tname
                            )
                        )
                    line += ftxt
                    ...
                    if len(dpcals) > 0:
                        ical = np.argmin(
                            np.abs(cnum1 - np.median(dpfluxs["FILE NUMBER"]))
                        )
                        # 						ical = np.argmin(np.abs(np.array(dpcals['FILE NUMBER'])-np.median(dpfluxs['FILE NUMBER'])))
                        cs = "{:.0f}-{:.0f}".format(cnum1[ical], cnum2[ical])
                    line += "\t{}\t{}\t{}\tflat{}.fits\twavecal{}.fits".format(
                        tname, str(dpfluxs["PREFIX"].iloc[0]), numberList(fnum), cs, cs
                    )
    ...
    if verbose == True:
        print(
            "\nBatch instructions written to {}, please this check over before proceeding".format(
                driver_file
            )
        )
...
def makeQApage(
    ...
    if isinstance(driver_input, str) == True:
        if os.path.exists(driver_input) == False:
            raise ValueError("Cannot find driver file {}".format(driver_input))
        try:
            driver = readDriver(driver_input)
        except:
            raise ValueError("Unable to read in driver file {}".format(driver_input))
    elif isinstance(driver_input, dict) == True:
        driver = copy.deepcopy(driver_input)
    else:
        raise ValueError(
            "Do not recognize format of driver input {}".format(driver_input)
        )
    for x in ["DATA_FOLDER", "CALS_FOLDER", "PROC_FOLDER", "QA_FOLDER"]:
        qa_parameters[x] = driver[x]

    # set up image folder
    if image_folder != "":
        image_folder += "/"
        if (
            os.path.exists(os.path.join(qa_parameters["QA_FOLDER"], image_folder))
            == False
        ):
            try:
                os.mkdir(os.path.join(qa_parameters["QA_FOLDER"], image_folder))
            except:
                print(
                    "WARNING: could not generate image folder {}; reverting to qa folder {}".format(
                        image_folder, qa_parameters["QA_FOLDER"]
                    )
                )
                image_folder = ""
    ...
    if isinstance(log_input, str) == True:
        if os.path.exists(log_input) == False:
            raise ValueError("Cannot find log file {}".format(log_input))
        if log_input.split(".")[-1] != "csv":
            raise ValueError(
                "Log input must be a csv file, you passed {}".format(log_input)
            )
        ...
        raise ValueError(
            "Do not recognize format of observing log input {}".format(log_input)
        )
    ...
    for x in ["TEMPLATE_FILE", "SOURCE_TEMPLATE_FILE"]:
        if os.path.exists(qa_parameters[x]) == False:
            raise ValueError(
                "Cannot find QA html template file {}".format(qa_parameters[x])
            )
    with open(qa_parameters["TEMPLATE_FILE"], "r") as f:
        index_txt = f.read()
    output_text, index_bottom = index_txt.split("[SOURCES]")
    with open(qa_parameters["SOURCE_TEMPLATE_FILE"], "r") as f:
        source_txt = f.read()
    with open(qa_parameters["SINGLE_PLOT_TEMPLATE_FILE"], "r") as f:
        single_txt = f.read()

    # replace relevant parameters in top of index
    output_text = output_text.replace(
        "[CSS_FILE]", os.path.basename(qa_parameters["CSS_FILE"])
    )
    output_text = output_text.replace("[LOG_HTML]", log_html_name)
    # NEED TO UPDATE WEATHER LINK
    output_text = output_text.replace(
        "[WEATHER_HTML]", qa_parameters["MKWC_ARCHIVE_URL"]
    )
    ...
    for sci in scikeys:
        ...
        if len(sci_log) > 0:
            ...
            # nicer coordinates
            s = SkyCoord(
                sci_log["RA"].iloc[0] + " " + sci_log["DEC"].iloc[0],
                unit=(u.hourangle, u.deg),
            )
            coord = s.to_string("hmsdms", sep=":")
            desig = "J" + s.to_string("hmsdms", sep="", pad=True).replace(
                ".", ""
            ).replace(" ", "")
            ...
            if "fixed" in sci_param["TARGET_TYPE"]:
                ...
                stxt = stxt.replace(
                    "[TARGET_ALADIN_URL]",
                    '[<a href="https://aladin.cds.unistra.fr/AladinLite/?target={}&fov=0.12&survey=CDS%2FP%2F2MASS%2Fcolor" target="_blank">Aladin</a>]'.format(
                        desig.replace("+", "%2B")
                    ),
                )
                stxt = stxt.replace(
                    "[TARGET_SIMBAD_URL]",
                    '[<a href="https://simbad.cds.unistra.fr/simbad/sim-coo?Coord={}&CooFrame=FK5&CooEpoch=2000&CooEqui=2000&CooDefinedFrames=none&Radius=2&Radius.unit=arcmin&submit=submit+query&CoordList=" target="_blank">SIMBAD</a>]'.format(
                        desig.replace("+", "%2B")
                    ),
                )
                ...
                stxt = stxt.replace(
                    "[COORDINATE]",
                    "{} at MJD {}".format(coord, str(sci_log["MJD"].iloc[0])),
                )
                stxt = stxt.replace("[TARGET_ALADIN_URL]", "")
                stxt = stxt.replace("[TARGET_SIMBAD_URL]", "")
                stxt = stxt.replace(
                    "[SBDB]",
                    '[<a href="https://ssd.jpl.nasa.gov/tools/sbdb_lookup.html#/?sstr={}" target="_blank">JPL SBDB</a>]'.format(
                        sci_param["TARGET_NAME"].replace(" ", "")
                    ),
                )
            stxt = stxt.replace(
                "[STD_SIMBAD_URL]",
                '[<a href="https://simbad.u-strasbg.fr/simbad/sim-id?Ident={}&NbIdent=1&Radius=2&Radius.unit=arcmin&submit=submit+id" target="_blank">SIMBAD</a>]'.format(
                    sci_param["STD_NAME"].replace(" ", "+")
                ),
            )
            stxt = stxt.replace("[DESIGNATION_URL]", desig.replace("+", "%2B"))
            stxt = stxt.replace(
                "[UT_START]", str(sci_log["UT_TIME"].iloc[0]).split(".")[0]
            )
            stxt = stxt.replace(
                "[UT_END]", str(sci_log["UT_TIME"].iloc[-1]).split(".")[0]
            )
            ...
            stxt = stxt.replace(
                "[INTEGRATION]", "{:.1f}".format(np.nansum(sci_log["INTEGRATION"]))
            )
        ...
        if len(sci_log) > 0:
            ...
            stxt = stxt.replace(
                "[STD_TYPE]", "({})".format(str(sci_log["SIMBAD_TYPE"].iloc[0]))
            )
        ...
        imfile = glob.glob(
            os.path.join(
                qa_parameters["QA_FOLDER"],
                "{}{}{}".format(
                    driver["CALIBRATED_FILE_PREFIX"], fsuf, qa_parameters["PLOT_TYPE"]
                ),
            )
        )
        if len(imfile) == 0:
            imfile = glob.glob(
                os.path.join(
                    ...
                    image_folder,
                    "{}{}{}".format(
                        driver["CALIBRATED_FILE_PREFIX"],
                        fsuf,
                        qa_parameters["PLOT_TYPE"],
                    ),
                )
            )
        ...
        if len(imfile) > 0:
            ptxt += (
                copy.deepcopy(single_txt)
                .replace(
                    "[IMAGE]", os.path.join(image_folder, os.path.basename(imfile[0]))
                )
                .replace("[IMAGE_WIDTH]", str(qa_parameters["IMAGE_WIDTH"]))
            )
            fitsfile = os.path.join(
                qa_parameters["PROC_FOLDER"],
                os.path.basename(imfile[0]).replace(".pdf", ".fits"),
            )
            if os.path.exists(fitsfile) == True:
                ptxt = ptxt.replace(
                    "[FITS]",
                    '[<a href="{}" download="{}">{}</a>]'.format(
                        fitsfile, os.path.basename(fitsfile), os.path.basename(fitsfile)
                    ),
                )
            else:
                ptxt.replace("[FITS]", "")
        ...
        for x in ["TARGET", "STD"]:
            ...
            imfile = glob.glob(
                os.path.join(
                    qa_parameters["QA_FOLDER"],
                    "{}{}*_raw{}".format(
                        driver["COMBINED_FILE_PREFIX"], fsuf, qa_parameters["PLOT_TYPE"]
                    ),
                )
            )
            if len(imfile) == 0:
            ...
            if len(imfile) > 0:
                ptxt += (
                    ...
                    .replace(
                        "[IMAGE]",
                        os.path.join(image_folder, os.path.basename(imfile[0])),
                    )
                    .replace("[IMAGE_WIDTH]", str(qa_parameters["IMAGE_WIDTH"]))
                )
            imfile = glob.glob(
            ...
            indexinfo = {
                ...
                "prefix": driver["SPECTRA_FILE_PREFIX"],
                "suffix": "",
                "extension": ".pdf",
            }
            files = make_full_path(
                qa_parameters["QA_FOLDER"],
                sci_param["{}_FILES".format(x)],
                indexinfo=indexinfo,
            )
            files = list(filter(lambda x: os.path.exists(x), files))
            if len(files) == 0:
                files = make_full_path(
                    os.path.join(qa_parameters["QA_FOLDER"], image_folder),
                    sci_param["{}_FILES".format(x)],
                    indexinfo=indexinfo,
                )
                files = list(filter(lambda x: os.path.exists(x), files))
            ...
            ptxt = copy.deepcopy(qa_parameters["HTML_TABLE_HEAD"])
            for i, f in enumerate(files):
                if i > 0 and np.mod(i, qa_parameters["NIMAGES"]) == 0:
                    ptxt += " </tr>\n <tr> \n"
                ptxt += (
                    copy.deepcopy(single_txt)
                    .replace("[IMAGE]", os.path.join(image_folder, os.path.basename(f)))
                    .replace("[IMAGE_WIDTH]", str(qa_parameters["IMAGE_WIDTH"]))
                )
            ptxt += copy.deepcopy(qa_parameters["HTML_TABLE_TAIL"])
            ...
            cnt = 0
            for f in fnums:
                if cnt > 0 and np.mod(cnt, qa_parameters["NIMAGES"]) == 0:
                    ptxt += " </tr>\n <tr> \n"
                imfile = glob.glob(
                    os.path.join(
                        qa_parameters["QA_FOLDER"],
                        "{}{}.a_*_trace{}".format(
                            driver["SCIENCE_FILE_PREFIX"],
                            str(f).zfill(setup.state["nint"]),
                            qa_parameters["PLOT_TYPE"],
                        ),
                    )
                )
            ...
            ptxt += copy.deepcopy(qa_parameters["HTML_TABLE_TAIL"])
            stxt = stxt.replace(
                "[{}_TRACE_APERTURE_FILES]".format(x.split("_")[0]), ptxt
            )
    ...
    for cs in cal_sets:
        ...
        imfile = glob.glob(
            os.path.join(
                qa_parameters["QA_FOLDER"],
                "flat{}_locateorders{}".format(cs, str(qa_parameters["PLOT_TYPE"])),
            )
        )
        if len(imfile) == 0:
            imfile = glob.glob(
        ...
        if len(imfile) > 0:
            imfile.sort()
            if qa_parameters["PLOT_TYPE"] == ".pdf":
                loopstr += '  <td align="center">\n   <embed src="{}" width={} height={}>\n   <br>{}\n   </td>\n'.format(
                    os.path.join(image_folder, os.path.basename(imfile[0])),
                    str(qa_parameters["IMAGE_WIDTH"]),
                    str(qa_parameters["IMAGE_WIDTH"]),
                    os.path.join(image_folder, os.path.basename(imfile[0])),
                )
            else:
    ...
    for cs in cal_sets:
        if cnt > 0 and np.mod(cnt, qa_parameters["NIMAGES"]) == 0:
            loopstr += " </tr>\n <tr> \n"
        imfile = glob.glob(
            os.path.join(qa_parameters["QA_FOLDER"], "wavecal{}_shift.pdf".format(cs))
        )
        if len(imfile) == 0:
            imfile = glob.glob(
                os.path.join(
    ...
    try:
        ...
        raise ValueError(
            "Unable to write QA page to {}; check path and permissions".format(outfile)
        )
    ...
    if image_folder != "":
        imfiles = glob.glob(
            os.path.join(
                qa_parameters["QA_FOLDER"], "*{}".format(qa_parameters["PLOT_TYPE"])
            )
        )
        for f in imfiles:
            ...
            shutil.move(
                f,
                os.path.join(
                    qa_parameters["QA_FOLDER"], image_folder, os.path.basename(f)
                ),
            )
    ...
    if output_folder != "" and output_folder != qa_parameters["QA_FOLDER"]:
        if os.path.exists(output_folder) == True:
            try:
                shutil.copytree(qa_parameters["QA_FOLDER"], output_folder)
            except:
                print(
                    "WARNING: could not copy contents of {} into {}; check path or permissions".format(
                        qa_parameters["QA_FOLDER"], output_folder
                    )
                )
    ...
    if verbose == True:
        print(
            "\nAcesss QA page at {}\n".format(
                os.path.join(qa_parameters["QA_FOLDER"], qa_parameters["FILENAME"])
            )
        )
...
def batchReduce(parameters, verbose=ERROR_CHECKING):
    ...
    """
    # default parameters
    if "VERBOSE" not in list(parameters.keys()):
    ...
    ps.pyspextool_setup(
        ...
        raw_path=parameters["DATA_FOLDER"],
        cal_path=parameters["CALS_FOLDER"],
        proc_path=parameters["PROC_FOLDER"],
        qa_path=parameters["QA_FOLDER"],
        qa_extension=parameters["PLOT_TYPE"],
    ...
    if parameters["CALIBRATIONS"] == True:
        for cs in cal_sets:
            ...
            indexinfo = {
                "nint": setup.state["nint"],
                "prefix": parameters["FLAT_FILE_PREFIX"],
                "suffix": ".a",
                "extension": ".fits",
            }
            temp_files = make_full_path(
                parameters["DATA_FOLDER"], cs, indexinfo=indexinfo
            )
            input_files = list(filter(lambda x: os.path.exists(x), temp_files))
            if len(input_files) == 0:
                raise ValueError(
                    "Cannot find any of the flat files {}".format(temp_files)
                )
            fnum = [
                int(
                    os.path.basename(f)
                    .replace(parameters["FLAT_FILE_PREFIX"], "")
                    .replace(".a.fits", "")
                    .replace(".b.fits", "")
                )
            ...
            if (
                os.path.exists(
                    os.path.join(parameters["CALS_FOLDER"], "flat{}.fits".format(cs))
                )
                == False
                or parameters["OVERWRITE"] == True
            ):
                logging.info("Creating flat{}.fits".format(cs))
                ps.extract.make_flat(
                    [parameters["FLAT_FILE_PREFIX"], fstr],
                    "flat{}".format(cs),
                    qa_show=parameters["qa_show"],
                    qa_write=parameters["qa_write"],
                    verbose=parameters["VERBOSE"],
                )
            else:
                if parameters["VERBOSE"] == True:
                    print(
                        "\nflat{}.fits already created, skipping (or use overwrite to remake)".format(
                            cs
                        )
                    )
            ...
            indexinfo = {
                "nint": setup.state["nint"],
                "prefix": parameters["ARC_FILE_PREFIX"],
                "suffix": ".a",
                "extension": ".fits",
            }
            temp_files = make_full_path(
            ...
            if len(input_files) == 0:
                raise ValueError(
                    "Cannot find any of the arc files {}".format(temp_files)
                )
            fnum = [
                int(
                    os.path.basename(f)
                    .replace(parameters["ARC_FILE_PREFIX"], "")
                    .replace(".a.fits", "")
                    .replace(".b.fits", "")
                )
                for f in input_files
            ...
            if (
                os.path.exists(
                    os.path.join(parameters["CALS_FOLDER"], "wavecal{}.fits".format(cs))
                )
                == False
                or parameters["OVERWRITE"] == True
            ):
                ps.extract.make_wavecal(
                    [parameters["ARC_FILE_PREFIX"], fstr],
                    "flat{}.fits".format(cs),
                    "wavecal{}".format(cs),
                    qa_show=parameters["qa_show"],
                    qa_write=parameters["qa_write"],
                ...
                if parameters["VERBOSE"] == True:
                    print(
                        "\nwavecal{}.fits already created, skipping (or use overwrite to remake)".format(
                            cs
                        )
                    )
    ...
    bkeys = list(
        filter(lambda x: OBSERVATION_SET_KEYWORD not in x, list(parameters.keys()))
    )
    scikeys = list(
        filter(lambda x: OBSERVATION_SET_KEYWORD in x, list(parameters.keys()))
    )
    if len(scikeys) == 0:
        if parameters["VERBOSE"] == True:
            print("No science files to reduce")
        return
    ...
    if parameters["EXTRACT"] == True:
        for k in scikeys:
            spar = parameters[k]
            for kk in bkeys:
                if kk not in list(spar.keys()):
                    spar[kk] = parameters[kk]
            ...
            ps.extract.load_image(
                [
                    spar["TARGET_PREFIX"],
                    extract_filestring(spar["TARGET_FILES"], "index")[:2],
                ],
                spar["TARGET_FLAT_FILE"],
                spar["TARGET_WAVECAL_FILE"],
                reduction_mode=spar["REDUCTION_MODE"],
                flat_field=True,
                linearity_correction=True,
                qa_show=spar["qa_show"],
                qa_write=spar["qa_write"],
                verbose=spar["VERBOSE"],
            )
            ...
            ps.extract.make_spatial_profiles(
                qa_show=spar["qa_show"],
                qa_write=spar["qa_write"],
                verbose=spar["VERBOSE"],
            )
            ...
            ps.extract.locate_aperture_positions(
                spar["NPOSITIONS"],
                method=spar["APERTURE_METHOD"],
                qa_show=spar["qa_show"],
                qa_write=spar["qa_write"],
                verbose=spar["VERBOSE"],
            )
            ...
            ps.extract.define_aperture_parameters(
                spar["APERTURE"],
                psf_radius=spar["PSF_RADIUS"],
                bg_radius=spar["BACKGROUND_RADIUS"],
                bg_width=spar["BACKGROUND_WIDTH"],
                qa_show=spar["qa_show"],
                qa_write=spar["qa_write"],
            )
            ...
            if len(fnum) > 0:
                ps.extract.do_all_steps(
                    [spar["TARGET_PREFIX"], "{}".format(numberList(fnum))],
                    verbose=spar["VERBOSE"],
                )
            ...
            ps.extract.load_image(
                [
                    spar["STD_PREFIX"],
                    extract_filestring(spar["STD_FILES"], "index")[:2],
                ],
                spar["STD_FLAT_FILE"],
            ...
            if len(fnum) > 0:
                ps.extract.do_all_steps(
                    [spar["STD_PREFIX"], "{}".format(numberList(fnum))],
                    verbose=spar["VERBOSE"],
                )
    ...
    if parameters["COMBINE"] == True:
        for k in scikeys:
            ...
            if (
                os.path.exists(
                    os.path.join(
                        parameters["PROC_FOLDER"],
                        "{}{}.fits".format(spar["COMBINED_FILE_PREFIX"], fsuf),
                    )
                )
                == False
                or parameters["OVERWRITE"] == True
            ):
                ps.combine.combine_spectra(
                    ["spectra", spar["TARGET_FILES"]],
                    "{}{}".format(spar["COMBINED_FILE_PREFIX"], fsuf),
                    scale_spectra=True,
                    scale_range=spar["SCALE_RANGE"],
                ...
                if parameters["VERBOSE"] == True:
                    print(
                        "\n{}{}.fits already created, skipping (or use overwrite to remake)".format(
                            spar["COMBINED_FILE_PREFIX"], fsuf
                        )
                    )
            ...
            if (
                ...
                ps.combine.combine_spectra(
                    ["spectra", spar["STD_FILES"]],
                    "{}{}".format(spar["COMBINED_FILE_PREFIX"], fsuf),
                    scale_spectra=True,
                    scale_range=spar["SCALE_RANGE"],
                    correct_spectral_shape=True,
                    qa_show=spar["qa_show"],
                    qa_write=spar["qa_write"],
                    verbose=spar["VERBOSE"],
                )
    ...
    if parameters["FLUXTELL"] == True:
        for k in scikeys:
            ...
            if (spar["TARGET_TYPE"].split("-")[0]).strip() == "fixed":
                ...
                if spar["VERBOSE"] == True:
                    print(
                        "\nTarget type {} not recognized, skipping telluric correction and flux calibration".format(
                            spar["TARGET_TYPE"].split("-")[0]
                        )
                    )
            ...
            if (
                os.path.exists(
                    os.path.join(
                        parameters["PROC_FOLDER"],
                        "{}{}.fits".format(spar["CALIBRATED_FILE_PREFIX"], tsuf),
                    )
                )
                == False
                or parameters["OVERWRITE"] == True
                ...
                ps.telluric.telluric_correction(
                    "{}{}.fits".format(spar["COMBINED_FILE_PREFIX"], tsuf),
                    spar["STD_NAME"],
                    "{}{}.fits".format(spar["COMBINED_FILE_PREFIX"], csuf),
                    "{}{}".format(spar["CALIBRATED_FILE_PREFIX"], tsuf),
                    standard_data=standard_data,
                    qa_show=spar["qa_show"],
                    qa_write=spar["qa_write"],
                    verbose=spar["VERBOSE"],
                    overwrite=spar["OVERWRITE"],
                )
            else:
                if parameters["VERBOSE"] == True:
                    print(
                        "\n{}{}.fits already created, skipping (or use overwrite to remake)".format(
                            spar["CALIBRATED_FILE_PREFIX"], tsuf
                        )
                    )
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print(
            "Call this function from the command line as python batch.py [data_folder] [cals_folder] [proc_folder]"
        )
    else:
        ...
        if len(sys.argv) > 4:
            ...
            log_file = sys.argv[3] + "log.html"
        print("\nWriting log to {}".format(log_file))
        writeLog(dp, log_file)
        if len(sys.argv) > 5:
            driver_file = sys.argv[5]
        else:
            driver_file = sys.argv[3] + "driver.txt"
        print("\nWriting batch driver file to {}".format(driver_file))
        writeDriver(
            dp,
            driver_file,
            data_folder=sys.argv[1],
            cals_folder=sys.argv[2],
            proc_folder=sys.argv[3],
        )
        txt = input(
