...
def wavecal_solution_1dxd(orders, line_info, home_order, dispersion_degree,
    ...
    fit = poly_fit_2d(line_info['x'], line_info['order'], scaled_wavelengths,
                      dispersion_degree, order_degree,
    ...
    if qafileinfo is not None:
        ...
        for i in range(norders):
            ...
            axes1.scatter(line_info['order'][z], residuals[z],
                          color=(red[i], 0, blue[i]), edgecolors='black',
                          s=8 ** 2, alpha=0.8)

            bad = line_info['goodbad'] == 0

            axes1.plot(line_info['order'][bad], residuals[bad], 's',
            ...
            axes2.scatter(line_info['x'][z], residuals[z],
                          color=(red[i], 0, blue[i]), edgecolors='black',
                          s=8 ** 2, alpha=0.8)

            axes2.plot(line_info['x'][bad], residuals[bad], 's',
                       markersize=13, markerfacecolor='none', color='black')
            ...
            axes1.text(0.02, 0.95, label, color='black', ha='left', va='top',
                       multialignment='left', transform=axes1.transAxes)

            label = 'Not all bad data points may be shown.'
            axes1.text(0.98, 0.95, label, color='black', ha='right', va='top',
