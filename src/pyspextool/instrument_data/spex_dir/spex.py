...
def load_data(file, linearity_info, keywords=None, linearity_coefficients=None):
    ...
    readtime = hdul[0].header["TABLE_MS"] / 1000.0
    divisor = hdul[0].header["DIVISOR"]

    #  Get set up for error propagation

    rdvar = (2.0 * readnoise**2) / ndrs / coadds / itime**2 / gain**2
    crtn = 1.0 - readtime * (ndrs**2 - 1.0) / 3.0 / itime / ndrs
    ...
    bitmask = (
        raw_img / divisor > linearity_info["max"] * 2 ** linearity_info["bit"]
    ).astype(np.uint8)
    ...
    if linearity_coefficients is not None:
        ...
        img = correct_linearity(
            raw_img / divisor, itime, slowcnt, ndrs, linearity_coefficients
        )
...
def read_fits(
    ...
    if linearity_correction is True:
        linearity_file = os.path.join(
            setup.state["instrument_path"], "spex_lincorr.fits"
        )
        try:
            linearity_coeffs = fits.getdata(linearity_file)
        except OSError as e:
            msg = f"""
            Could not open the linearity correction file: {linearity_file}.
            Please check that the file downloaded from Git LFS properly. 
            It should be 16.8 MB.
    ...
    if pair_subtract is True:
        ...
        for i in range(0, nimages):
            ...
            combmask = math.combine_flag_stack(
                np.stack((a[3], b[3])), nbits=linearity_info["bit"] + 1
            )
